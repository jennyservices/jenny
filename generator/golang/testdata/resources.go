...
func makeGetUserEndpoint(svc TestService2, opts *options.Options) endpoint.Endpoint {
...
// endpoints as used in https://gokit.io/examples/stringsvc.html#endpoints
func makeLoginEndpoint(svc TestService2, opts *options.Options) endpoint.Endpoint {
	loginEndpoint := func(ctx context.Context, request interface{}) (interface{}, error) {

		resp := _loginResponse{}
		var err error

		resp.Location, err = svc.Login(ctx)
	...
	loginMiddleware = endpoint.Chain(loginMiddleware, auth.RequireScopes([]string{"pets"}))
...
// NewTestService2HTTPServer returns a HTTPHandler that can be used with the go std http package
func NewTestService2HTTPServer(svc TestService2, opts ...options.Option) http.Handler {
	r := mux.NewRouter()

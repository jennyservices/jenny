...
type ResponseRecorder struct {
	...
	wroteHeader  bool
	shadowWriter http.ResponseWriter
}
...
func (rw *ResponseRecorder) Write(buf []byte) (int, error) {
	//defer rw.shadowWriter.Write(buf)
...
func (rw *ResponseRecorder) WriteHeader(code int) {
	//defer rw.shadowWriter.WriteHeader(code)
	if rw.wroteHeader {
		return
	}
...
func responseHeaders(resp *http.Response) []v1.Header {
	headers := []v1.Header{}
...
func requestHeaders(req *http.Request) []v1.Header {
	headers := []v1.Header{}
...
func parseResponse(resp *http.Response) v1.Response {
...
func parseRequest(req *http.Request) v1.Request {
	bytz, _ := ioutil.ReadAll(req.Body)
	return v1.Request{
		Method:  req.Method,
		URL:     req.URL.String(),
...
func (ds *debugService) saveResponse(id string, resp *http.Response) {
	if trp, ok := ds.trips[id]; ok {
		trp.finish = time.Now()
		trp.resp = parseResponse(resp)

		err := ds.db.Update(func(tx *bolt.Tx) error {
			requestsBucket, err := tx.CreateBucketIfNotExists(bucketName)
			if err != nil {
				return err
			}
			...
			return requestsBucket.Put([]byte(id), buf.Bytes())
		})
		if err != nil {
			log.Fatal(err)
		}
